...
// use shared function to get list of interview questions BY CATEGORY
// 	var listFromFirebase = getListFromFirebase(window.name);
// generate html list stuff
// 	new Category("a", listFromFirebase);
// look up our jquery element from our table!
// 	var el = categoriesToElements[window.name];
// actually add in the random question to our element/page
// 	el.html(.... .randomquestion());
	
	var ref = new Firebase('https://z5npagh32i1.firebaseio-demo.com/');
	
	if (window.name == "Interview"){
		
		ref.orderByChild("name").equalTo("Interview").once("value", function(snapshot) {
			var obj = snapshot.val();
			var interviewquestions = Object.keys(obj).map(function (key) { return obj[key].text; });
